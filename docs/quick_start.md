...
### Triton-family AMD Gluon candidate path

Gluon stays inside the existing Triton route. Do **not** introduce a new
`kernel_type`; keep `kernel_type = triton`.

- For Triton kernels, GEAK defaults to `gluon_feature_mode=auto` and includes
  AMD Gluon in the candidate search space.
- `gluon_feature_mode` is mainly an ablation/debug control:
  - `off` = plain Triton-only, no-Gluon ablation
  - `auto` = default candidate search across plain Triton and AMD Gluon
  - `force` = require an AMD Gluon output path
- The input dialect is normally inferred and can be overridden only when needed:
  - `plain_triton`
  - `nv_gluon`
  - `amd_gluon`
- By default, GEAK prefers an `amd_gluon` candidate first when structurally
  viable, then lets the benchmark choose between it and plain Triton.
- For `nv_gluon` inputs, the agent is expected to translate NVIDIA-specific
  interfaces or assumptions before optimizing on AMD.

You usually do not need to pass Gluon metadata explicitly. Use overrides only
...
python -m minisweagent.run.preprocess.preprocessor /path/to/kernel.py \
  ...
  --gluon-feature-mode off \
  --target-backend hip/gfx942
```

Typical Triton optimization can stay simple:
