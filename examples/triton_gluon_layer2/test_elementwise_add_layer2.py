...
AMD_EXAMPLE = ROOT / "03_elementwise_add_amd.py"
...
AMD = _load_module(AMD_EXAMPLE, "layer2_elementwise_add_amd")
...
def run_compile_case():
    ...
    compiled = AMD.elementwise_add(a, b, c)
    torch.cuda.synchronize()
    print(f"COMPILE_OK=1")
...
def _benchmark_case(xnumel, ynumel, iterations, seed=0):
    ...
    ms = triton.testing.do_bench(lambda: AMD.elementwise_add(a, b, c), rep=iterations)
...
def test_correctness_layer2(xnumel, ynumel):
    run_correctness_case(xnumel, ynumel)
...
def main():
    parser = argparse.ArgumentParser(description="Focused Layer 2 harness for elementwise_add on gfx942.")
