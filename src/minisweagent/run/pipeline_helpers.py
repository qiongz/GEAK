...
def _build_gluon_working_set(
    ...
    *,
    ...
    lines = [
        ...
        "- This working set applies to every task that received Gluon worker docs/context, including L0, L1, variant, hybrid/mixed, and existing AMD Gluon paths. Plain/Base tasks without Gluon worker docs keep the normal task flow.",
        "- Before the first edit, write a `Patch evolution plan` in strategy notes. It must state the current task type, the anchor or safe baseline, the allowed scope, the expected first patch, and the pass/fail rule for the next patch.",
        "- `patch_0` should prove the smallest allowed executed anchor for generated L0, preserve and refine the verified anchor for L1/variant/hybrid, or preserve the production source contract for existing AMD Gluon.",
        "- If `patch_N` compiles, executes, and passes correctness, `patch_N+1` may take the normal-improvement track: change exactly one allowed variable, component, layout parameter, launch constant, memory path, matrix subpath, or dispatch condition.",
        "- If `patch_N` fails, `patch_N+1` must take the failure-fix track: classify the current failure layer and fix only that layer before adding any new optimization.",
        "- For whole-kernel or compile-risk Gluon tasks, `patch_0` is compile/execute only. Do not tune performance until the anchor runs and passes correctness.",
        ...
        "- After each patch, record `Changed component`, `Expected effect`, `Observed effect`, `Keep / revert / continue`, and whether the next patch follows `normal improvement` or `failure fix`.",
        "- Use `60_real_patterns.md::failure_to_next_patch_map` as the default failure-layer to next-patch mapping when the task does not provide a more specific mapping.",
        ...
        "- Before editing, compare the task body, task metadata, source code, and routed docs. Check that `Target component` matches `minimum_executable_unit`, `Allowed change` matches the declared execution path, matrix/dot risks match `matrix_lowering_required` or failure-layer notes, and `Plain competitor` names the same direction/component when one is required.",
        "- If `failure_layers` or `expected_failure_layers` are missing, incomplete, or inconsistent with the source, do not invent new metadata fields. Record `Task correction: <reason> -> <corrected failure layer order>` in strategy notes/summary and continue within the existing task boundary.",
        "- Soft corrections may narrow the next patch to the observed failure layer, add missing layout/matrix/reduction/wrapper layers to notes, or mark a scoped path as infeasible under the task's own policy. They must not widen the allowed scope or change the benchmark contract.",
        "- Stop and report instead of editing only when the existing hard contract is impossible to satisfy: the required plain competitor is a different target/direction, the only implementation touches forbidden scope, or the allowed execution path cannot produce an executed Gluon result under `scope_infeasible_policy`.",
...
def _build_forced_triton_gluon_skill_context() -> list[str]:
    """Return compact triton-gluon skill guidance for required AMD Gluon tasks."""
    ...
    lines = [
        ...
        "Before editing or save_and_test, view every absolute path listed in the REQUIRED BEFORE EDITING OR SAVE_AND_TEST block.",
        "",
        ...
        "Keep scope to one subpath/component unless the task explicitly says `bundle_allowed=true`. For L0, `patch_0` is the smallest correctness anchor; later patches follow the pass/fail rule: passing patches may make one normal improvement, failing patches fix only the current failure layer.",
