...
def _build_gluon_working_set(feature_metadata: dict[str, Any] | None) -> list[str]:
    ...
    lines = [
        ...
        "- Before editing a Gluon path, write a `Gluon implementation plan` in your strategy notes. Include: task scope, parent layouts, every `tl.arange` replacement, tensor-creation layouts, broadcast/SliceLayout pairs, matrix path (or none), and module wiring.",
    ...
    lines.extend(
        [
            ...
            "  - A required AMD Gluon patch must not leave `tl.arange`, `tl.zeros`, `tl.full`, `tl.load`, `tl.store`, `tl.where`, or `tl.dot` in the edited `@gluon.jit` subpath.",
