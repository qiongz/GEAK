...
def _build_gluon_working_set(feature_metadata: dict[str, Any] | None) -> list[str]:
    ...
    lines = [
        ...
        "- Before editing a layout-heavy Gluon path, write down the layout plan in your strategy notes: logical parent layouts, the `SliceLayout` used for each `[:, None]` / `[None, :]` tensor, and which indices must not be reused across parent layouts.",
        ...
        "- For Extension L1 memory/buffer lowering, start from the best correctness-passing Gluon/mixed anchor and preserve its layout plan. If no Gluon anchor passed, shrink to a layout/memory smoke path instead of wrapping the original plain Triton body.",
