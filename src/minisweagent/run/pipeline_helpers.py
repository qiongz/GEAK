...
def _build_gluon_working_set(
    ...
    *,
    ...
    lines = [
        ...
        "- This task received Gluon worker docs/context. Before editing, write a `Patch evolution plan` and use `10_search_policies.md::atomic_component_lattice` plus `60_real_patterns.md::patch_evolution_by_task_type` / `failure_to_next_patch_map` for pass/fail next-patch rules.",
        ...
        "- Before editing, compare the task body, metadata, source, and routed docs. Use `10_search_policies.md::overlay_direction_vs_mechanism` / `atomic_component_lattice` for same-direction component checks and `60_real_patterns.md::Task consistency check` / `Whole-kernel L0 comparison anchor` / `l0_scope_by_kernel_family` for whole-kernel anchors.",
