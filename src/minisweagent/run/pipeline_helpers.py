...
def _build_gluon_working_set(feature_metadata: dict[str, Any] | None) -> list[str]:
    ...
    lines = [
        ...
        "- Before editing a Gluon path, write a `Gluon implementation plan` in your strategy notes. Include: task scope, target symbol if the task names a stage/helper, parent layouts, every `tl.arange` replacement, tensor-creation layouts, broadcast/SliceLayout pairs, device scalar/math `tl.*` -> `gl.*` mapping, buffer `other`/`stored_value` dtype contract, matrix path (or none), and module wiring.",
        ...
        "- For Extension L0 on layout-heavy kernels, target one compileable subpath first; do not translate the whole attention/decode/GEMM body in one patch.",
        "- For Extension L1 memory/buffer or matrix lowering, start from the best correctness-passing Gluon/mixed anchor and preserve its layout plan. If no Gluon anchor passed, shrink to a layout/memory smoke path instead of wrapping the original plain Triton body.",
        ...
        "- For hybrid/mixed tasks, keep a visible host-side dispatch and preserve the Base path for shapes or sub-operations where Base wins.",
    ...
    lines.extend(
        [
            ...
            "  - Stage-specific L1 tasks must touch the named target symbol; editing a different stage is target mismatch even if it benchmarks.",
