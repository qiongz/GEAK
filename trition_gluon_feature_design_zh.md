...
- `docs/triton_gluon.md` 保留为完整背景与备份长文，但 agent 真正执行时优先使用 split-docs。
...
- `scaled_matrix_lowering` / `tdm_descriptor_path` / `async_shared_pipeline`
  ...
  - 意义：这些来自真实 Triton/Aiter Gluon 例子，但属于后置 refinement；必须有 L0 anchor、operator-local source，或 existing AMD Gluon production path 支持。
...
- `Base + Extension L0 overlay`
  ...
  - 合同：L0 必须写 `Plain competitor`、`Source Base family`、`Target component`、`minimum_executable_unit`、`allowed_execution_path`。
